...
router.get('/', verifyToken, async (req, res) => {
    try {
        const items = await db.all('SELECT * FROM shop_items ORDER BY type, price');
...
router.post('/purchase', verifyToken, async (req, res) => {
    try {
        ...
        // Get the item
        const item = await db.get('SELECT * FROM shop_items WHERE id = ?', [item_id]);
...
router.post('/verify-purchase', verifyToken, async (req, res) => {
    try {
        ...
        // Get item details
        const item = await db.get('SELECT * FROM shop_items WHERE id = ?', [product_id]);
